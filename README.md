...
This fork adds **Script Studio** to the existing Screenshot-to-Code UI. From the start
screen, click **Script → Scene Prompts**. Paste a script, choose output type, clip
duration, aspect ratio and prompt theme, then scan. Gemini turns the script into a
scene-by-scene storyboard, estimates total duration, and prints standalone image/video
prompts in copy-friendly code blocks.

For real video generation, configure:
- `REPLICATE_API_KEY`
- `VIDEO_MODEL=owner/model`

The selected Replicate model must accept `prompt`, `duration`, and `aspect_ratio` input
fields. Different video models expose different input schemas, so choose a compatible
model in Script Studio Settings. API keys entered in the browser are used for that
request and are not hard-coded into the repository.
